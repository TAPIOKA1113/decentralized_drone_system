...
const int DataRate = 54; //データレート[Mbps] //6,9,12,18,24,36,48,54 既定値
int DataLength = 32; //パケット長[byte]
int Payload = 2304; //最大ペイロード[byte]
...
if ((t + this->time_offset) % int(ControlInterval/TimeStep) == 0) {
    ...
    //自身の位置情報＋次周期における自身の位置情報＋制御周期内に受け取った推定位置情報のパケットを送信し，受信パケットをクリeア
    auto packet = std::make_shared<Packet>(this->agent->id, this->x, this->y, tilde_x, tilde_y);
    //転送する位置情報は予測位置情報のみ
    //ランダムに受信した位置情報のみを転送する
    int transfer_count  = 0;
    int i = 0;
    std::uniform_int_distribution<int> dist(0, 99); //0-99までの乱数を生成
    for (auto &packet_rx : this->com->packets_rx) {
    int random_value = dist(rnd);
    int prob = 50;
    if(random_value > prob - 1){
        transfer_count += 1;
...
for (int i=0; i < this->robots.size(); i++) {
    ...
    // ロボット同士の距離を算出
    double distance = std::hypot(this->robots[i]->x - this->robots[j]->x, this->robots[i]->y - this->robots[j]->y);
    if (distance > ConsensusRange) return false; // 1台でも範囲外ならFalse
    }
}
return true;
}
